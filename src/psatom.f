...
c debug
!           write(ie,*) 'it state',it,i,n(i),l(i),m(i),e(i)
            mup=max(mup,nin)
            ninu(i) = nin
c
c
     ...
     1                  vxc(1,1),eexc,ex,ec,.false.)
            ...
            if(tkli_first) then
              do iorb=1,nc+nv
                ...
                mp(iorb)=1
!               write(iu,*) '%psatom: iorb',iorb,f(iorb),fp(iorb)
!               write(iu,*) '             ',iorb,n(iorb)
!               write(iu,*) '             ',iorb,l(iorb)
              enddo
              norb_kli=0
              do iorb=1,nc+nv
              ...
              enddo
!             write(iu,'(/a,1x,i2)')
!    1          ' %psatom: kli - number of occupied orbitals = ',
!    1          norb_kli
            endif
            do i=1,mmax
              dens(i)=0.5d0*rho(i)
...
!             write(iu,*) '%psatom: iexckli',vckli(1),vckli(100)
              do ir=1,mmax
