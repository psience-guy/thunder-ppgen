c $Header: /cvs/ppgen/src/vklix.f,v 1.2 2006/08/30 21:22:57 jelen Exp $
c
...
!              if(icount .lt. 2) then
!                  print *,
!    + i,j,'  ',l(i),l(j),ll,l(i)+l(j)+ll,cj,acgc(l(j),l(i),ll)
!                endif
...
!     print *, '--- vkli corrector matrix ---'
!     do i=1,iorb-1
!       print *, i, (sm(i,j),j=1,iorb-1)
!     enddo
...
!     print *, '--- vkli average difference potential ---'
!     print *, 'iorb',iorb
!     do i=1,iorb
!       print *, i, n(i),l(i),m(i),f(i),sv(i)
!     enddo

!     print *, '--- vkli verify solution ---'
!     do i=1,iorb-1
!       test=zero
!       do j=1,iorb-1
!         test=test+smsave(i,j)*sv(j)
!       enddo
!       print *, i, test,svsave(i)
!     enddo
      else if (mode .ne. 2) then
        print *, '& - vklix - warning: unimplemented mode',mode
      endif

!     print *, '--- vkli orbital correctors ---'
!     do i=1,iorb-1
!       print *, i, sv(i)
!     enddo
...
!         rewind(iunit)
!         do j=1,mup
!           write(iunit,*) r(j),f(i)*ud0(j,i)*vorb(j,i)/dens(j)
!         enddo
!         close(iunit)
!       endif
