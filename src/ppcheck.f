c $Header: /cvs/ppgen/src/ppcheck.f,v 1.2 2006/08/30 21:22:56 jelen Exp $
c-----------------------------------------------------------------------
c pseudopotential
...
! jel: store kb-potential in a specific format into a files
!      we want to go through all l-components
        if (tkb) then  
     ...
     1        *uu(i,ll)/r(i)**2
! the specific format
            vkbX(i,ll) = (vbare(i,ll)-vbare(i,ll_loc)) * uu(i,ll)
          enddo
! store kb-potential into files
          junk = 'vkb'
          write (ext,'(i1,''.dat'')') ll-1
          junk = 'vkb'//ext
          write (*,*)  ' Write vkb L=',ll-1,' to ',junk,'. [a.u.]'
          open (unit=39, file=junk, status='unknown')
          do i = 1,mmax
           write (39,*) r(i), vkbX(i,ll)
          end do
          close (39)

          vaverage(ll)=dmelm(mmax,al,r,vkb(1,ll),uu(1,ll))
! ckb equals zero for local part 
          ckb = 0.0d0
          if (ll .ne. ll_loc ) ckb=1.d0/vaverage(ll)
          open (unit=39, file='qlclATOMIC.dat', status='unknown')
          do i = llbeg,ll
           if (i .lt. ll) read (39,*) ekb
           if (i .eq. ll) write (39,811) ckb,ll-1
811        format(f14.7,'                             L=',i2)
          enddo
        endif
!jel-end
